...
function setActiveToastYPos()
    local curYPos = ToastYPos
    for _, gui in pairs(activeToast) do
        gui.Position = UDim2.new(gui.Position.X.Scale, 0, curYPos, 0)
        curYPos = curYPos - 0.08
    end
end

function HRHelper.showToast(txt, time)
    local tstGUI = Instance.new("TextLabel")
    tstGUI.Size = UDim2.new(0.12, 0, 0.07, 0)
    tstGUI.AnchorPoint = Vector2.new(0, 0)
    tstGUI.Position = UDim2.new(-0.35, 0, ToastYPos, 0)
    tstGUI.BackgroundColor3 = Color.Background
	tstGUI.AutomaticSize = Enum.AutomaticSize.X
    tstGUI.TextSize = 14
    tstGUI.ZIndex = 15
	tstGUI.RichText = true
	tstGUI.TextColor3 = Color3.fromRGB(225, 225, 225)
    tstGUI.Parent = parent

    local tstPadding = Instance.new("UIPadding")
    tstPadding.PaddingLeft = UDim.new(0, 15)
    tstPadding.PaddingRight = UDim.new(0, 15)
    tstPadding.Parent = tstGUI

    local tstCorner = Instance.new("UICorner")
    tstCorner.CornerRadius = UDim.new(0, 10)
    tstCorner.Parent = tstGUI
		
	local tstStroke = Instance.new("UIStroke")
    tstStroke.Color = Color.Primary
    tstStroke.Thickness = 2
    tstStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    tstStroke.Parent = tstGUI

	local tstStroke2 = Instance.new("UIStroke")
    tstStroke2.Color = Color.Primary
    tstStroke2.Thickness = 1.2
    tstStroke2.Parent = tstGUI

	tstGUI.Visible = true
    tstGUI.Text = txt
    table.insert(activeToast, tstGUI)
    setActiveToastYPos()

	HRHelper:slide(tstGUI, "X", -0.35, 0.002, function()
        task.delay(time or 2, function()
            HRHelper:slide(tstGUI, "X", 0.002, -0.35, function()
                local index = table.find(activeToast, tstGUI)
                if index then
                    table.remove(activeToast, index)
                end
                setActiveToastYPos()
